...
public class TarefasController
{
    ...
    public class TarefasController : ControllerBase
    {
        ...
        [HttpGet]
        public ActionResult<List<Tarefas>> GetAll()
        {
            return Ok(_tarefaService.ObterTodos());
        }

        [HttpGet("{id}")]
        public ActionResult<TarefaDetalhadaDto> GetById(int id)
        {
            var tarefaDto = _tarefaService.ObterPorId(id);
            if (tarefaDto == null) return NotFound();
            return Ok(tarefaDto);
        }

        [HttpPost]
        public ActionResult<TarefaDetalhadaDto> Add(CriarTarefa tarefaDto)
        {
            try
            {
